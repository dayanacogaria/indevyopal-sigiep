...
<script type="text/javascript">
  $("#myModal1").modal('show');
    $("#ver1").click(function(){
        ...
        if(!empty($idCnt[0])){ 
            if(!empty($idPPAL[0])){ ?>
                window.location='<?php echo '../GP_RECAUDO_FACTURACION.php?recaudo='.md5($idPago[0]).'&cnt='. md5($idCnt[0]).'&pptal='. md5($idPPAL[0]).'&cupones='.$cupones.'&valor='.$valorCupones; ?>';
            <?php }else{ ?>
                window.location='<?php echo '../GP_RECAUDO_FACTURACION.php?recaudo='.md5($idPago[0]).'&cnt='. md5($idCnt[0]).'&cupones='.$cupones.'&valor='.$valorCupones; ?>';
           <?php } ?>            
        <?php }else{ ?>
            window.location='<?php echo '../GP_RECAUDO_FACTURACION.php?recaudo='.md5($idPago[0]).'&cupones='.$cupones.'&valor='.$valorCupones; ?>';
        <?php } ?>    
  });
</script>
<?php }else{ ?>
