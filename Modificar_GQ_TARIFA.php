...
if (!empty($_GET['id'])){
    ...
    $sqltar = $con->Listar("
    SELECT 
    fc.*, vh.nombre, aso.nombre, aso.valor
    FROM gq_fraccion fc
    ...
    WHERE md5(fc.id_unico) = '$id'");
    $id = $sqltar[0][0];
    $nombre = $sqltar[0][1];
    $valor = $sqltar[0][2];
    $unidadm = strtolower($sqltar[0][3]);
    $unidadm = ucfirst($unidadm);
    $desviacion = $sqltar[0][4];
    $intervalo = $sqltar[0][5];
    $tarifaaso = $sqltar[0][6];    
    $tipov = $sqltar[0][7];
    $nmtipov = $sqltar[0][8];
    $nmtarifaaso = $sqltar[0][9];
    $vltarifaaso = $sqltar[0][10];
}
?>
...
<body>
    <div class="container-fluid text-center">
        <div class="row content">
            ...
            <div class="col-sm-10 text-left" style="margin-top:-20px;">
                <h2 id="forma-titulo3" align="center" style="margin-right: 4px; margin-left: 4px;">Modificar tarifa</h2>
                    ...
                    <form id="form" name="form" class="form-horizontal" method="POST" enctype="" action="json/registrar_GQ_TARIFAJson.php">
                        ...
                        <div class="form-group" style="margin-top: -4px;">
                            ...
                            <div class="col-sm-5 col-md-5 col-lg-5">
                                <input  required="required" type="text" name="txtNombre" id="txtNombre" class="form-control" placeholder="Nombre" title="Registre nombre de la tarifa" style="width: 100%;" required autocomplete="off" value="<?php echo $nombre;?>">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="txtvalor" class="control-label col-sm-4 col-md-4 col-lg-4"><strong class='obligado'>*</strong>Valor:</label>
                            <div class="col-sm-5 col-md-5 col-lg-5">
                                <input type="text" name="txtvalor" id="txtvalor" class="form-control" title="Ingrese el valor"  placeholder="Valor" onkeypress="return txtValida(event, 'decimales')" required style='width:100%;' autocomplete="off" value="<?php echo $valor;?>">
                            </div>
                        </div>
                        ...
                        <div class="form-group" style="margin-top: 11px;">
                            ...
                            <div class="col-sm-5 col-md-5 col-lg-5" style="padding-top: 5px;">
                                <input type="text" name="txtintervalo" id="txtintervalo" class="form-control" title="Ingrese Intervalo"  placeholder="Intervalo" onkeypress="return txtValida(event, 'decimales')" required style='width:100%;' autocomplete="off" value="<?php echo $intervalo;?>">
                            </div>
                        </div>
                        <div class="form-group" style="height: 0px;">
                            ...
                            <div class="col-sm-5 col-md-5 col-lg-5">
                                <input type="text" name="txtdesviacion" id="txtdesviacion" class="form-control" title="Ingrese Desviación"  placeholder="Desviación" onkeypress="return txtValida(event, 'decimales')" required style='width:100%;' autocomplete="off" value="<?php echo $desviacion;?>">
                            </div>
                        </div>
                        <div class="form-group" style="margin-top: 6px;">
                            <label for="slttarifaaso" class="control-label col-sm-4 col-md-4 col-lg-4"><strong class='obligado'></strong>Tarifa Asociada:</label>
                            <div class="col-sm-5 col-md-5 col-lg-5">
                                <select name="slttarifaaso" id="slttarifaaso" class="select2_single form-control" title="Seleccione Tarifa Asociada">                                    
                                    <?php
                                        $where = '';
                                        if (!empty($tarifaaso)){
                                            $valor = number_format($vltarifaaso, 2, ',', '.');                                            
                                            echo "<option value='$tarifaaso' selected>$nmtarifaaso - $valor</option>";
                                            $where = 'WHERE id_unico NOT IN ('.$tarifaaso.','.$id.')';
                                        }else {
                                            echo "<option value=''>Tarifa Asociada</option>";
                                            $where = 'WHERE  id_unico <> '.$id;
                                        }                                        
                                        $html = "";                                            
                                        $sqper = "SELECT id_unico, nombre, valor FROM gq_fraccion $where";
                            ...
                            <div class="col-sm-5 col-md-5 col-lg-5">
                                <select name="slttipov" id="slttipov" class="select2_single form-control" title="Seleccione Tipo Vehiculo" required>
                                    <?php
                                        $where = '';
                                        if (!empty($tipov)){
                                            echo "<option value='$tipov' selected>$nmtipov</option>";
                                            $where = 'WHERE id_unico <> '.$tipov;
                                        }else {
                                            echo "<option value=''>Tipo Vehiculo</option>";
                                            $where = '';
                                        } 
                                        $html = "";                                            
                                        $sqper = "SELECT * FROM gp_tipo_vehiculo $where";
                                        $resper = $mysqli->query($sqper);
                                        while ($row = mysqli_fetch_row($resper)) {
                                            $vh = $row[1];
    ...
    <script>       
        $(document).ready(function () {
            ...
            let unidad = "<?php echo $unidadm ?>";
            var len = unidad.length;
            if (len > 0 ){
                $("#sltunidadm").val(unidad);
            }            
        });
