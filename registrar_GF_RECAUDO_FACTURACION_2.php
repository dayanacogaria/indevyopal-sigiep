                    ...
                    <form name="form" class="form-horizontal" method="POST" style="margin-left:30px"  enctype="multipart/form-data" action="json/registrarPagoFacturaJson.php">
                        ...
                        <div class="form-group form-inline" style="margin-top:-15px;margin-bottom:5px;">
                            ...
                            <select name="sltBuscar" id="sltBuscar" onchange="buscarPago()" class="form-control col-sm-1" style="width: 250px" title="Buscar recaudo">
                                <?php
                                echo "<option value=''>Recuado</option>";
                                ...
                                <select name="sltFactura2" id="sltFactura2" class="form-control input-sm" style="width:170px;height:30px;" title="Seleccione  factura" required="" onclick="load_value_input('',$('#sltFactura2').val())">
                                    ...
                                    #Inclusión nueva validamos que si existe un tercero que de ese tercero me traiga las facturas relacionadas que tengan saldo
                                        if(!empty($tercero)){
                                            ...
                                            $sqlF = "SELECT DISTINCT    fat.id_unico,fat.numero_factura,tpf.prefijo,DATE_FORMAT(fat.fecha_factura,'%d/%m/%Y') FROM gp_factura fat
                                            ...
                                            while($fila = mysqli_fetch_row($resultF)){
                                                list($factura, $fat, $pag, $xxx, $afec) = array($fila[0], 0, 0, 0,0);
                                                ...
                                                while($row_f = mysqli_fetch_row($res_f)){
                                                    ...
                                                    $sql_pa = "  SELECT      DISTINCT ((dtp.valor) + dtp.iva + dtp.impoconsumo) + dtp.ajuste_peso AS ULTIMO
                                                                FROM        gp_detalle_factura dtp 
                                                                WHERE       dtp.detalleafectado = $row_f[1]";
                                                    $res_pa = $mysqli->query($sql_pa);
                                                    while($row6= mysqli_fetch_row($res_pa)){
                                                         $afec += $row6[0];
                                                    }
                                                ...
                                                if($xxx > 0){
                                                    $val[$fila[0]]=$xxx;
                                                    echo '<option value="'.$fila[0].'">'.ucwords(mb_strtoupper($fila[2].' '.$fila[1].' Saldo: '.number_format($xxx,2, ',', '.').' '.$fila[3])).'</option>';
                                                }
                                            }
                                        }else{
