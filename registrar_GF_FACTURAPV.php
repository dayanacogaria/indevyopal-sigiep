...
if(!empty($_REQUEST['cotizacion'])){
    ...
    WHERE MD5(f.id_unico)='".$_REQUEST['cotizacion']."'");
    list($id_factura, $id_tercero, $tercero, 
    $fecha_factura, $fecha_vencimiento, $id_cc, $centro_costo, $descripcion, $id_estado_f, 
    $estado_factura,$id_vendedor, $vendedor, $cufe,$idfp, $fp,$idmp, $mp, $tcb,$id_uvms, $usuario, $cuotas, $abono )= 
    array($df[0][0], $df[0][4], $df[0][5],
    $df[0][6], $df[0][7], $df[0][8], $df[0][9], $df[0][10], $df[0][11], $df[0][12], 
    $df[0][13], $df[0][14],$df[0][15], $df[0][18], $df[0][19], $df[0][20],$df[0][21],$df[0][22], $df[0][23], $df[0][24].' - '.$df[0][25], $df[0][26], $df[0][27]  );
...
<html>
                ...
                <div class="col-sm-8 col-md-8 col-lg-8 text-left">
                    ...
                    <div style="margin-top: -7px; border:4px solid #020324; border-radius: 10px;" class="client-form col-sm-12 col-lg-12 col-md-12">
                        <form id="form" name="form" class="form-horizontal" method="POST" enctype="multipart/form-data" action="javaScript:guardarF()">
                            ...
                            <input type="hidden" name="cot" id="cot" value="<?= $coti; ?>" />
                            <input type="hidden" name="cot" id="cot" value="<?= $coti; ?>" />
    ...
    <div class="modal fade" id="myModalEliminar" role="dialog" align="center" >
        ...
        <script>
            ...
            $("#sltTipoCot").change(function(){
                 var tipoF= $("#sltTipoCot").val();
                 var form_data = {
                     estruc:37,
                     tipo:tipoF
                 };
                 $.ajax({
                     ...
                     data:form_data,
                     success: function (data) {
                         $("#sltNumeroC").html(data).fadeIn();
                         $("#sltNumeroC").css('display','none');
                     }
                 });
             });
             $("#sltNumeroC").change(function () {
                var cotiza=$("#sltNumeroC").val();
                 var form_data = {
                    estruc:38,
                     id:cotiza
                 };
