<?php
/**
 * controllerGFTipoDependencia.php
 *
 * Archivo de control para registro, modificación y eliminado de la tabla dependencia
 *
 * @author Alexander Numpaque
 * @package Tipo Dependencia
 * @param String $action Variable para indicar que proceso se va a realizar
 * @version $Id: controllerGFTipoDependencia.php 001 2017-05-25 Alexander Numpaque$
 */
...
require ('../json/registrarTipo_dependencia.php');
if($action == 'insert') {
	$nombre  = '"'.$_POST['nombre'].'"';
	$result = tipoDependencia::save_data($nombre);
	echo "<html>\n";
	echo "<head>\n";
	...
	if($result==true){
	  	...
	  	echo "\t\twindow.location='../listar_GF_TIPO_DEPENDENCIA.php';";
	  	echo "\t});";
	  	echo "</script>";
	}else{
	  	echo "<script type=\"text/javascript\">";
	...
	$nombre  = '"'.$_POST['nombre'].'"';
    $id_unico  = '"'.$_POST['id'].'"';
    $result = tipoDependencia::modify_data($nombre, $id_unico);
    echo "<html>\n";
	echo "<head>\n";
	echo "\t<meta charset=\"utf-8\">\n";
	echo "\t<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">\n";
	...
	$result = tipoDependencia::delete_data($id_unico);
