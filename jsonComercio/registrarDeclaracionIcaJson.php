<?php
   require_once '../Conexion/conexion.php';
    ...
    $V = mysqli_fetch_row($vig);
     ...
     $CDecla = "SELECT MAX(cod_dec) FROM  gc_declaracion WHERE cod_dec LIKE '$V[0]%' AND parametrizacionanno = '$V[0]' ";
    ...
    if($nde < 1 ){
        ...
        $X = 0;
        echo $sql = "INSERT INTO gc_declaracion (cod_dec, contribuyente, periodo, vigencia,  tipo_per, tipo_dec, fecha, correccion, tipo_correc, observaciones,clase,parametrizacionanno)
             VALUES ($numD, $c, $p, $vf, 4, 1, '$hoy', NULL, NULL, NULL,2,$pa)";
        ...
        if($pesasM == "1"){
            ...
            for($a = 1; $a < count($X); $a++){
                ...
                if( !empty($vvalor)){
                    ...
                    echo $ingreso="INSERT INTO gc_declaracion_ingreso (declaracion, act_comer, tipo_ingreso, valor, inmueble) VALUES ($idDeclaracion, NULL,3, $vvalor, $idv)";
            ...
            echo "<br/>";
            echo $ConDecS = "SELECT SUM(dd.valor) FROM gc_detalle_declaracion dd LEFT JOIN gc_concepto_comercial cc ON dd.concepto = cc.id_unico WHERE dd.declaracion = '$idDeclaracion' AND cc.tipo_ope = 2 AND dd.tipo_det = 1 ";
            $ValCons = $mysqli->query($ConDecS);
            $ValCS = mysqli_fetch_row($ValCons);

            echo "<br/>";
            echo $ConDecR12 = "SELECT SUM(dd.valor) FROM gc_detalle_declaracion dd LEFT JOIN gc_concepto_comercial cc ON dd.concepto = cc.id_unico WHERE dd.declaracion = '$idDeclaracion' AND cc.tipo_ope = 1 AND dd.tipo_det = 1 AND  cc.tipo = 7 ";
...
<div class="modal fade" id="LiqReca" role="dialog" align="center" >
        ...
        <div class="modal-content" style="width: 450px;">
            ...
            <div class="modal-body" >
                <div class="row text-left">
                    <form class="form-horizontal" id="formu" name="formu" method="post" action="registrarRecaudoIcaJson.php">                                                        
                        <div class="form-group" >
                            ...
                            <input type="hidden" name="txtTipo" value="2">
                        ...
                        <div class="form-group"  ">
                            <?php 
                                $Declaracion = "SELECT id_unico FROM gc_declaracion WHERE cod_dec = '$numD'";
    ...
    <script>
        $('#soloDec').click(function(e){
            $("#LiqReca").modal('hide');
            window.location = "../registrar_GC_ICA.php?fecDR=<?php echo $fecha_dec;?>&N=<?php echo md5($c);?>&per=<?php echo md5($p);?>&vig=<?php echo md5($vf);?> ";
        });
    </script>
</div>
