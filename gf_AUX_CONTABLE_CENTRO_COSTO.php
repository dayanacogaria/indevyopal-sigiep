...
?>
<title>Auxiliares Contables Centro Costo</title>
</head>
<body>
...
<style>
    label #sltTci-error, #sltTcf-error, #fechaini-error, #fechafin-error, #sltctai-error, #sltctaf-error, #sltCci-error, #sltCcf-error  {
    display: block;
    color: #155180;
    font-weight: normal;
    font-style: italic;
...
<div class="container-fluid text-center">
    ...
    <?php require_once 'menu.php'; ?>
        <div class="col-sm-8 text-left" style="margin-left: -16px;margin-top: -20px"> 
            <h2 align="center" class="tituloform">Auxiliares Contables Centro De Costo</h2>
            ...
            <?php 
                $cuentaF = "SELECT id_unico, codi_cuenta, nombre from gf_cuenta "
                        . "WHERE parametrizacionanno = $anno ORDER BY codi_cuenta DESC";
            ...
            <?php
                $rsctai="SELECT id_unico, sigla, nombre FROM gf_centro_costo "
                        . "WHERE parametrizacionanno = $anno  ORDER BY sigla ASC";
                $rsctai = $mysqli->query($rsctai);        
            ?>
            <div class="form-group" style="margin-top: 0px">
                <label for="sltCci" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Centro de Costo Inicial:</label>
                <select name="sltCci" id="sltCci" style="height: auto" class=" select2_single form-control" title=
                        "Seleccione Centro de Costo Inicial" required>
                    <option value="">Centro Costo Inicial</option>
                <?php 
                    while ($filactaI = mysqli_fetch_row($rsctai)) 
                { 
                ?>
                    <option value="<?php echo $filactaI[1];?>"><?php echo mb_strtoupper($filactaI[1]).' - '.ucwords(mb_strtolower($filactaI[2]));?></option>                                
                <?php 
                }
                 ?>                                    
                </select>
            ...
            <?php
                $rsctaf="SELECT id_unico,sigla,  nombre FROM gf_centro_costo "
                        . "WHERE parametrizacionanno = $anno ORDER BY sigla DESC";
                $rsctaf = $mysqli->query($rsctaf); 
              ?>
            <div class="form-group" style="margin-top: 0px">
                <label for="sltCcf" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Centro de Costo Final:</label>
                <select name="sltCcf" id="sltCcf" style="height: auto" class=" select2_single form-control" title=
                        "Seleccione Centro de Costo Final" required>
                    <option value="">Centro Costo Final</option>
                <?php 
                    while ($filactaf = mysqli_fetch_row($rsctaf)) 
                { 
                ?>
                    <option value="<?php echo $filactaf[1];?>"><?php echo mb_strtoupper($filactaf[1]).' - '.ucwords(mb_strtolower($filactaf[2]));?></option>                                
                <?php 
                }
                 ?>                                    
                </select>
...
function reporteExcel(){
   $('form').attr('action', 'informes/generar_INF_AUX_CNT_COS_EXCEL.php');
}

</script>
<script>
function reportePdf(){
    $('form').attr('action', 'informes/generar_INF_AUX_CONT_CENTRO_COSTO.php');
}
