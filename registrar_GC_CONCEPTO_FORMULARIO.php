...
<title>Registrar Concepto Formulario</title>
</head>
...
<!-- Llamado al menu del formulario -->    
  <?php require_once 'menu.php'; ?>
    ...
    <div class="col-sm-10 text-left">
      ...
      <h2 id="forma-titulo3" align="center" style="margin-bottom: 10px; margin-right: 4px; margin-left: 4px;">Registrar Concepto Formulario</h2>
      <!--volver-->
...
<!-- inicio del formulario --> 
          <form name="form" class="form-horizontal" method="POST"  enctype="multipart/form-data" action="jsonComercio/registrarConceptoFormularioJson.php">
          ...
          <p align="center" style="margin-bottom: 25px; margin-top: 25px; margin-left: 30px; font-size: 80%">Los campos marcados con <strong style="color:#03C1FB;">*</strong> son obligatorios.</p>
            ...
            <div class="form-group" style="margin-top: -10px;">
              <label for="codigo" class="col-sm-5 control-label">Código:</label>
                <input type="text" name="codigo" id="codigo" class="form-control" maxlength="15" title="Ingrese el Código" onkeypress="return txtValida(event, 'car')" placeholder="Código" >
            </div>  

             <div class="form-group" style="margin-top: -10px;">
                 <label for="renglon" class="col-sm-5 control-label"><strong class="obligado">*</strong>Renglón:</label>
                <input type="text" name="renglon" id="renglon" class="form-control" maxlength="15" title="Ingrese el Renglón" onkeypress="return txtValida(event,'num')" placeholder="Renglón" required >
            </div>

            <?php
            $cuentaI = "SELECT id_unico,codigo,descripcion FROM gc_concepto_comercial";
                ...
                <select  name="conceptoComercial" id="sector"  style="height: auto" class="select2_single form-control" title="Seleccione Sector" required="">
                    ...
                    <option value="">Concepto Comercial</option>
                        ...
                        <?php while($row=mysqli_fetch_array($rsctai)){ ?>
                                     <option value="<?php echo $row['id_unico']?>"><?php echo ucwords(mb_strtolower($row['codigo']."-".$row['descripcion'])); ?></option>
                        <?php } ?>
                </select>
            </div><br>
