...
##################################################
 ...
 ?>
<title>Auxiliares Contables Proyecto</title>
</head>
<body>
...
<div class="container-fluid text-center">
    ...
    <?php require_once 'menu.php'; ?>
        <div class="col-sm-8 text-left" style="margin-left: -16px;margin-top: -20px"> 
            <h2 align="center" class="tituloform">Auxiliares Contables Proyecto</h2>
            ...
            <?php 
                $sqlpyi = "SELECT * FROM gf_proyecto WHERE compania = $compania";
                $rspyi = $mysqli->query($sqlpyi);
            ?>
            <div class="form-group" style="margin-top: 0px">
                <label for="sltproyectoI" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Proyecto Inicial:</label>
                <select name="sltproyectoI" id="sltproyectoI" style="height: auto" class=" select2_single form-control" title=
                        "Seleccione Proyecto Inicial" required>
                    <option value="">Proyecto Inicial</option>
                <?php 
                    while ($rowi = mysqli_fetch_row($rspyi)) 
                { 
                ?>
                    <option value="<?php echo $rowi[0];?>"><?php echo ucwords(mb_strtolower($rowi[1]));?></option>                                
                <?php 
                }
                 ?>                                    
                </select>
            ...
            <?php 
                $sqlpyf = "SELECT * FROM gf_proyecto WHERE compania =$compania ORDER BY id_unico DESC";
                $rspyf = $mysqli->query($sqlpyf);
            ?>
            <div class="form-group" style="margin-top: 0px">
                <label for="sltproyectoF" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Proyecto Final:</label>
                <select name="sltproyectoF" id="sltproyectoF" style="height: auto" class=" select2_single form-control" title=
                        "Seleccione Proyecto Final" required>
                    <option value="">Proyecto Final</option>
                <?php 
                    while ($row = mysqli_fetch_row($rspyf)) 
                { 
                ?>
...
function reporteExcel(){
   $('form').attr('action', 'informes/generar_INF_AUX_CNT_EXCEL_PROYECTO.php');
}

function reportePdf(){
    $('form').attr('action', 'informes/generar_INF_AUX_CONTABLES_PROYECTO.php');
}
