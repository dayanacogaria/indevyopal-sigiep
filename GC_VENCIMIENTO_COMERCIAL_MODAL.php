...
if (isset($_POST['id'])){
 ...
 $sql="SELECT vc.id_unico,
             ...
             DATE_FORMAT(vc.fecha_final,'%d/%m/%Y') AS fechaInicio,
             DATE_FORMAT(vc.fecha_inicio_inter,'%d/%m/%Y') AS fechaInicio,
             DATE_FORMAT(vc.fecha_limite_decl,'%d/%m/%Y') AS fechaInicio,
             vc.vigencia,
             ac.vigencia
      FROM  gc_vencimiento_comercial vc
      LEFT JOIN gc_anno_comercial ac ON ac.id_unico=vc.vigencia 
      WHERE vc.id_unico=$id";
...
<script>
    ...
    $(function(){
        ...
        $("#fechaInicialm").datepicker({changeMonth: true,});
        $("#fechaFinalm").datepicker({changeMonth: true});
        $("#fechaInicioInteresm").datepicker({changeMonth: true});
        $("#fechaLimiteDeclaracionm").datepicker({changeMonth: true});
...
<div class="modal fade modalvc" id="mdlModificar" role="dialog" align="center" aria-labelledby="mdlModificar" aria-hidden="true">
    <div class="modal-dialog" style="height:580px;">
        <div class="modal-content">
            
            <div id="forma-modal" class="modal-header">
                <h4 class="modal-title"  style="font-size: 24; padding: 3px;">Modificar Vencimiento Comercial</h4>
          ...
          <div class="modal-body" style="margin-top: 8px">
            ...
            <form name="form" class="form-inline" method="POST"  enctype="multipart/form-data" action="jsonComercio/modificarVencimientoComercialJson.php" >
                       
                   <div style="width: 500px;padding-left: -50%;" >


                        <div  class="form-group" style="margin-top: 5px;">
                            <label for="ff"  class="col-sm-3"><strong class="obligado">*</strong>Fecha Inicial:</label>
                             <input  type="text" name="fechaInicial" onkeypress="return justNumbers(event);" id="fechaInicialm"  class="form-control" required="" value="<?php echo $row[1] ?>" />
                        </div>


                        <div  class="form-group" style="margin-top: 5px;">
                            <label for="ff"  class="col-sm-3"><strong class="obligado">*</strong>Fecha Final:</label>
                            <input  type="text" name="fechaFinal" onkeypress="return justNumbers(event);" id="fechaFinalm"  class="form-control"  required="" value="<?php echo $row[2] ?>"/>
                        </div>

                        <div  class="form-group" style="margin-top: 5px;">
                            <label for="ff"  class="col-sm-3"><strong class="obligado">*</strong>Fecha Interes:</label>
                            <input style="margin-left: -1.1%" type="text" name="fechaInicioInteres" onkeypress="return justNumbers(event);" id="fechaInicioInteresm"  class="form-control"  required="" value="<?php echo $row[3] ?>"/>
                        </div>

                        <div  class="form-group" style="margin-top: 5px;">
                            <label for="ff"  class="col-sm-4"><strong class="obligado">*</strong>Fecha Limite Declaración:</label>
                            <input  type="text" name="fechaLimiteDeclaracion" onkeypress="return justNumbers(event);" id="fechaLimiteDeclaracionm"  class="form-control"  required="" style="margin-left: -14.1%;" value="<?php echo $row[4] ?>"/>
                        </div><br><br><br>
                        <?php $vigenc=$row[5];
                              $vc=$row[0];
                         ?>
                        ...
                        <input type="hidden" name="idvc" value="<?php echo $vc ?>">



                        <div class="form-group" style="margin-left: -8%;">
                            <button type="submit" id="btnGuardarDetalle" class="btn btn-primary sombra"><li class="glyphicon glyphicon-floppy-disk" style="margin-left: -2%;"></li></button>                                
