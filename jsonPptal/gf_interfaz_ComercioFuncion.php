...
switch ($_REQUEST['action']){
    ...
    case (1):
        ...
        if(count($comp)>0){
            #********** Buscar Pagos Segun Fechas *********#
            $pg =$con->Listar("SELECT DISTINCT  rc.id_unico FROM gc_recaudo_comercial  rc 
               WHERE rc.clase = 1 AND rc.parametrizacionanno = $parm_anno AND rc.fecha BETWEEN '$fechaI' AND '$fechaF' ");
            for ($p = 0; $p < count($pg); $p++) {
                $idPago =$pg[$p][0];
    ...
    case (2):
        ...
        #********** Buscar Pagos Segun Fechas *********#
        $pg =$con->Listar("SELECT id_unico FROM gc_recaudo_comercial  
                WHERE clase = 1 AND parametrizacionanno = $parm_anno AND fecha BETWEEN '$fechaI' AND '$fechaF' ");
        for ($p = 0; $p < count($pg); $p++) {
            $idPago = $pg[$p][0];
