...
$anno = $_SESSION['anno'];
?>
<title>Libro Auxiliar Presupuestal de Reservas y Cuentas Por Pagar</title>
</head>
<body>
    ...
    <style>
        label  #sltctaf-error, #fechaini-error, #fechafin-error, #sltctai-error,#sltClasep-error, #fuente-error{
            display: block;
            color: #bd081c;
            font-weight: bold;
            font-style: italic;
    ...
    <div class="container-fluid text-center">
        ...
        <?php require_once 'menu.php'; ?>
            <div class="col-sm-10 text-left" style="margin-left: -16px;margin-top: -20px"> 
                <h2 align="center" class="tituloform">Libro Auxiliar Presupuestal de Reservas y Cuentas Por Pagar</h2>
                <div style="border: 4px solid #020324; border-radius: 10px; margin-left: 4px; margin-right: 4px;" class="client-form">
                    <form name="form" id="form" class="form-horizontal" method="POST"  enctype="multipart/form-data" action="#" target=”_blank”>  
                        ...
                        <?php
                            $clasePptal = "SELECT id_unico,nombre FROM gf_tipo_clase_pptal
                                        WHERE id_unico IN (15,16)";
                            $rsclasePptal = $mysqli->query($clasePptal);
                            ?>
                            <div class="form-group" style="margin-top: -10px">
                                <label for="sltClasep" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Clase Presupuestal:</label>
                                <select name="sltClasep" id="sltClasep"  style="height: auto" class="select2_single form-control" title="Seleccione Clase Presupuestal" required="required">
                                    <option value="">Clase Presupuestal</option>
                                    <?php
                                    while ($filaClaseP = mysqli_fetch_row($rsclasePptal)) {
                                        ?>
                                        <option value="<?php echo $filaClaseP[0]; ?>"><?php echo ucwords(mb_strtolower($filaClaseP[1])); ?></option>                                
                                        <?php
                                    }
                                    ?>                                    
                                </select>
        ...
        <script>
            function reporteExcel() {
                $('form').attr('action', 'informes/INF_AUX_CPP_RSR.php?c=2');
            }

        </script>
        <script>
            function reportePdf() {
                $('form').attr('action', 'informes/INF_AUX_CPP_RSR.php?c=1');
            }
        </script>
    </div>
</body>
