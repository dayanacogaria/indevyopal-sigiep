...
$mod = "SELECT
  l.id_unico, 
  l.unidad_vivienda_medidor_servicio, 
  l.periodo, 
  p.nombre, 
  m.referencia,
  l.valor
FROM
  gp_lectura l
LEFT JOIN
  gp_unidad_vivienda_medidor_servicio uvms ON l.unidad_vivienda_medidor_servicio = uvms.id_unico
LEFT JOIN
  gp_medidor m ON uvms.medidor = m.id_unico
LEFT JOIN
  gp_periodo p ON l.periodo = p.id_unico
WHERE md5(l.id_unico)= '$id'"; 
$mod = $mysqli->query($mod);
...
<div class="container-fluid text-center">
  <div class="row content">
    ...
    <!--Titulo del formulario-->
      <h2 id="forma-titulo3" align="center" style="margin-bottom: 20px; margin-right: 4px; margin-left: 4px;">Modificar Lectura</h2>

      <div style="border: 4px solid #020324; border-radius: 10px; margin-left: 4px; margin-right: 4px;" class="client-form">

          <form name="form" id="form" accept-charset=""class="form-horizontal" method="POST"  enctype="multipart/form-data" action="json/modificar_GP_LECTURAJson.php">

          <p align="center" style="margin-bottom: 25px; margin-top: 25px; margin-left: 30px; font-size: 80%">Los campos marcados con <strong style="color:#03C1FB;">*</strong> son obligatorios.</p>
          <input type="hidden" name="id" id="id" value="<?php echo $mod[0];?>">
          <!--Ingresa la información-->
            <div class="form-group" style="margin-top: -10px;">
                <input type="hidden" id="iduvms" name="iduvms" required title="Ingrese Referencia" value="<?php echo $mod[1]; ?>"/>
              <label for="iduvms" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Referencia:</label>
              <input type="text" name="uvms" id="uvms" class="form-control" title="Ingrese la referencia"  placeholder="Referencia" required style="display: inline; width: 250px" value="<?php echo $mod[4]?>" onchange="valorCambio();">
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) { 
                                        ...
                                        if (resultado == 'null' || resultado== null || resultado =='' || resultado ==""){
                                            ...
                                            var idA = <?php echo $mod[1];?>;
                                            if(resultado!=idA){ 
                                                 var form_data={
                                                    case:15,
                                                    iduvms:id,
                                                    periodo:<?php echo $mod[2];?>
                                                };
                                                $.ajax({
                                                    type: 'POST',
                                                    url: "consultasBasicas/busquedas.php",
                                                    ...
                                                    success: function (data) { 
                                                        var resultado = JSON.parse(data);
                                                        if(resultado=='1'){
                                                            $("#periodo1 option[value='<?php echo $mod[2]?>']").hide();
                                                            document.getElementById('periodo').value='';
                                                            document.getElementById('periodo1').value='';
                                                        }else {
                                                           document.getElementById('periodo').value='<?php echo $mod[2]?>';
                                                           document.getElementById('periodo1').value='<?php echo $mod[2]?>'; 
                                                        }
                                                    }
                                                });  
            ...
            <div class="form-group" style="margin-top: -10px;">
              ...
              <?php $period= "SELECT DISTINCT "
                            . "p.id_unico, p.nombre FROM gp_periodo p "
                            . "WHERE NOT EXISTS(SELECT * FROM gp_lectura l "
                                    . "WHERE l.periodo = p.id_unico AND unidad_vivienda_medidor_servicio ='$mod[1]') "
                            . "ORDER BY p.nombre ASC";
              $period = $mysqli->query($period);?>
              <input type="hidden" name="periodo" required id="periodo"  title="Seleccione periodo" value="<?php echo $mod[2]?>">
              <select style="width:250px;" name="periodo1"  id="periodo1"  class="select2_single form-control" title="Seleccione Periodo" >
                  <option value="<?php echo $mod[2]?>"><?php echo ucwords(strtolower($mod[3]))?></option>
                  <?php while ($rowP = mysqli_fetch_row($period)){ ?>
                  <option value="<?php echo $rowP[0]?>"><?php echo ucwords(strtolower($rowP[1]));?></option>
                  <?php } ?>
                </select>
            </div>
            <div class="form-group" style="margin-top: 0px;">
              <label for="valor" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Valor:</label>
              <input type="text" name="valor"  id="valor" class="form-control" title="Ingrese el valor"  value="<?php echo $mod[5]?>" placeholder="Valor" required style="width: 250px">
                                ...
                                <tbody id="cuerpo" class="text-center"> 
                                    ...
                                    <?php while($row = mysqli_fetch_row($resultado)) { ?>
                                        <tr>
                                            ...
                                            <td class="campos"><?php echo ucwords(strtolower(($row[3])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[4])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[5])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[6])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[7])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[8])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[9])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[10])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[11])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[12])));?></td>
                                            <td class="campos"><?php echo ucwords(strtolower(($row[13])));?></td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
...
<script>
    function habilitar(){
        var id= document.getElementById('uvms').value;
        var form_data={
            case:8,
            id:id
        };
        $.ajax({
            ...
            data:form_data,
            success: function (data) { 
                document.getElementById('iduvms').value=data;
            }
        });
