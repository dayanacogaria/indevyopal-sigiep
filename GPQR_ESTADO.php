...
if(empty($_GET['id'])) {
    ...
    $titulo = "Modificar ";
    $id     = $_GET['id'];
    $row    = $con->Listar("SELECT * FROM gpqr_estado
    ...
    <div class="container-fluid text-center">
        <div class="row content">
            ...
            <div class="col-sm-10 text-left">
                <h2 id="forma-titulo3" align="center" style="margin-bottom: 20px; margin-right: 4px; margin-left: 4px;"><?php echo $titulo.' Estado'?></h2>
                <a href="listar_GPQR_ESTADO.php" class="glyphicon glyphicon-circle-arrow-left" style="display:inline-block;margin-left:10px; font-size:150%; vertical-align:middle;text-decoration:none" title="Volver"></a>
                ...
                <div style="border: 4px solid #020324; border-radius: 10px; margin-left: 4px; margin-right: 4px;" class="client-form">
                    ...
                    <script>
                        function registrar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "jsonPQR/gpqr_estadoJson.php?action=2",
                                data:formData,
                                contentType: false,
                                processData: false,
                                    ...
                                    if(response==1){
                                        ...
                                        $("#Aceptar").click(function(){
                                            if(PQR == 1){
                                                ...
                                                document.location='listar_GPQR_ESTADO.php';
                                            }
                                        })
                                    } else {
                    ...
                    <form name="form" id="form" class="form-horizontal" method="POST"  enctype="multipart/form-data" >
                        ...
                        <div class="form-group" style="margin-top: 20px;">
                            <label for="no" class="col-sm-5 control-label"></label>
                            <button type="button" onclick="javascript:modificarES();"  class="btn btn-primary sombra" style=" margin-top: -10px; margin-bottom: 10px; margin-left:0px">Modificar</button>
                        </div>
                        <input type="hidden" name="MM_insert" >
                    </form>
                    <script>
                        function modificarES(){
                            jsShowWindowLoad('Modificando Datos ...');
                            var formData = new FormData($("#form")[0]);
                            $.ajax({
                                type: 'POST',
                                url: "jsonPQR/gpqr_estadoJson.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
                                success: function(response)
                                {
                                    ...
                                    if(response==1){
                                        ...
                                        $("#mensaje").html('La edición del estado se encuentra Restringida');
                                        $("#modalMensajes").modal("show");
                                        $("#Aceptar").click(function(){
                                            $("#modalMensajes").modal("hide");
                                            document.location='listar_GPQR_ESTADO.php';
