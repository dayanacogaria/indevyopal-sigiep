...
#05/04/2017 --- Nestor B --- se agrego el atributo mb para que tome las tildes
    ...
    #LISTAR
    $queryCC = "SELECT dc.id_unico, dc.documento, dc.caracteristica, c.nombre, td.nombre "
            . "FROM gg_confi_caracteristica dc "
            . "LEFT JOIN gg_caracteristica c ON dc.caracteristica = c.id_unico "
            . "LEFT JOIN gf_tipo_dato td ON c.tipo_dato = td.id_unico "
            . "WHERE md5(dc.documento)= '$id1'"; 
	...
	<div class="row content">
            ...
            <div class="col-sm-10 text-left" style="margin-top:-10px">	
              ...
              <a href="<?php echo $_SESSION['url'];?>" class="glyphicon glyphicon-circle-arrow-left" style="display:inline-block;margin-left:10px; font-size:150%; margin-top:-5px;vertical-align:middle;text-decoration:none" title="Volver"></a>
                  ...
                  <h5 id="forma-titulo3a" align="center" style="width:92%; display:inline-block; margin-bottom: 10px; margin-right: 4px; margin-left: 4px; margin-top:-20px;  background-color: #0e315a; color: white; border-radius: 5px">Documento:<?php echo ucwords((strtolower($rowD['nombre']))); 
          ?></h5>

                <div style="border: 4px solid #020324; border-radius: 10px; margin-left: 4px; margin-right: 4px;" class="client-form">         
                    <form name="form" id="form"  class="form-horizontal" method="POST"  enctype="multipart/form-data" action="jsonProcesos/registrar_GG_DOCUMENTO_CARACTJson.php">
                        ...
                        <div align="center" class="form-group form-inline" style="margin-top:-20px; margin-left:0px">
                            <input type="hidden" name="documento" id="documento" value="<?php echo $rowD['id_unico']?>">
            ...
            <select style="display:inline-block; width:250px; margin-bottom:15px; height:40px" name="caracteristicam" id="caracteristicam" class="select2_single form-control" title="Seleccione característica" required>
                <?php while ($modTer = mysqli_fetch_row($caracteristica1)) { ?>
                      <option value="<?php echo $modTer[0]; ?>">
                        <?php echo ucwords((mb_strtolower($modTer[1]).' - '.$modTer[2])); ?>
                      </option>
                <?php  
...
<script type="text/javascript">
      function eliminar(id)
      {
         ...
         $("#ver").click(function(){
              ...
              $.ajax({
                  type:"GET",
                  url:"jsonProcesos/eliminar_GG_DOCUMENTO_CARACTJson.php?id="+id,
          ...
          function modificarItem()
    {
      ...
      var documento= document.getElementById('documentom').value;
      var caracteristica= document.getElementById('caracteristicam').value;
      
      $.ajax({
        type:"GET",
        url:"jsonProcesos/modificar_GG_DOCUMENTO_CARACTJson.php?id="+id+"&documento="+documento+"&caracteristica="+caracteristica,
        success: function (data) {
          result = JSON.parse(data);
        if(result=='3'){
