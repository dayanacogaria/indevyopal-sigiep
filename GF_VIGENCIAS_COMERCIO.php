...
$titulo2 ="";
if(empty($_GET['id'])){
    $titulo = "Registrar Vigencias Interfaz Comercio";
    $titulo2 =".";
} else {
    $titulo = "Modificar Vigencias Interfaz Comercio";
    $sql = "SELECT id_unico, nombre, valor, vigencias_anteriores "
            . "FROM gf_vigencias_interfaz_comercio "
...
<script>
    function registrar(){
        ...
        var form_data = { action: 1, nombre:$("#nombre").val(), valor:$("#valor").val(), vigencias_anteriores : va};
        $.ajax({
          type: "POST",
...
<script>
    function modificar(){
        var va = $('input:radio[name=vigencias_anteriores]:checked').val();
        var form_data = { action: 2, id:$("#id").val(),nombre:$("#nombre").val(), valor:$("#valor").val(), vigencias_anteriores : va};
...
<script>
    $("#aceptar").click(function(){
       window.location="LISTAR_GF_VIGENCIAS_COMERCIO.php";
    });
</script>
