<?php
  ...
  $cadena3 = strtolower($observaciones);
  ...
  $queryUA="SELECT nombre, observaciones, elemento_flujo FROM gg_fase "
          . "WHERE id_unico = '$id'";
  $carA = $mysqli->query($queryUA);
  $numA=  mysqli_fetch_row($carA);
  $cadena2 = strtolower($numA[0]);
  $cadena4 = strtolower($numA[1]);
  
  $queryU="SELECT * FROM gg_fase "
          . "WHERE LOWER(nombre) = '$nombre' "
          . "AND LOWER(observaciones)='$observaciones' "
          . "AND elemento_flujo ='$elemento'";
 ...
 $obC1= preg_replace($exp_regular, $cadena_nueva, $cadena3);
 $obC2= preg_replace($exp_regular, $cadena_nueva, $cadena4);
  
 //comparación para que guarde
   if($nombreC2==$nombreC1 && $obC1 ==$obC2 && $numA[2]==$elemento){
        
         $update = "UPDATE gg_fase "
                 . "SET nombre ='$nombre', "
                 . "observaciones = '$observaciones', "
                 . "elemento_flujo= '$elemento'  "
                 . "WHERE id_unico = '$id'";
...
<html>
    ...
    <?php if($resultado =='3') { ?>
        <script type="text/javascript">
          ...
          $("#ver3").click(function(){
            $("#myModal3").modal('hide');
            window.location='../LISTAR_GG_FASE.php';
          });
        </script>
    <?php } else { ?>
