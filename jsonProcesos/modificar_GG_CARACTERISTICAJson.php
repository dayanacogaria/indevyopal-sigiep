<?php
  ...
  $queryUA="SELECT nombre, tipo_dato FROM gg_caracteristica "
          . "WHERE id_unico = '$id'";
  $carA = $mysqli->query($queryUA);
  $numA=  mysqli_fetch_row($carA);
  $cadena2 = strtolower($numA[0]);
  $queryU="SELECT * FROM gg_caracteristica "
          . "WHERE LOWER(nombre) = LOWER('$nombre') AND "
          . "tipo_dato = '$tipo'";
 ...
 $nombreC2= preg_replace($exp_regular, $cadena_nueva, $cadena2);
   ...
   if($nombreC2==$nombreC1 && $numA[1]==$tipo){
        $update = "UPDATE gg_caracteristica "
              . "SET nombre ='$nombre', tipo_dato= '$tipo', unidad=$unidad "
              . "WHERE id_unico = '$id'";
...
<html>
    ...
    <?php if($resultado =='3') { ?>
        <script type="text/javascript">
          ...
          $("#ver3").click(function(){
            $("#myModal3").modal('hide');
            window.location='../LISTAR_GG_CARACTERISTICA.php';
          });
        </script>
    <?php } else { ?>
