<?php
  ...
  $porcentaje  = $mysqli->real_escape_string(''.$_POST['numero'].'');
  
  $queryU="SELECT * FROM gg_persona_proceso "
          . "WHERE tercero= $tercero  AND proceso = $proceso";
  $car = $mysqli->query($queryU);
  $num=mysqli_num_rows($car);

  if($num == 0)
  {
   $insert = "INSERT INTO gg_persona_proceso (porcentaje_participacion, proceso, tercero) "
          . "VALUES($porcentaje, $proceso, $tercero)";
...
<html>
    ...
    <?php if($resultado==true){ ?>
        <script type="text/javascript">
          ...
          $("#ver1").click(function(){
            $("#myModal1").modal('hide');
            window.location='../GG_TERCERO_ASOCIADO.php?id=<?php echo md5($proceso);?>';
          });
        </script>
    <?php }else{ ?>
        <script type="text/javascript">
