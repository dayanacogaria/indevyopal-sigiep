<?php
  ...
  $queryUA="SELECT nombre FROM gg_unidad_tiempo "
          . "WHERE id_unico = '$id'";
  $carA = $mysqli->query($queryUA);
  $numA=  mysqli_fetch_row($carA);
  $cadena2 = strtolower($numA[0]);
  
  $queryU="SELECT * FROM gg_unidad_tiempo "
 ...
 //comparación para que guarde
   if($nombreC2==$nombreC1){
         ...
         $update = "UPDATE gg_unidad_tiempo "
              . "SET nombre ='$nombre'  "
              . "WHERE id_unico = '$id'";
...
<html>
    ...
    <?php if($resultado =='3') { ?>
        <script type="text/javascript">
          ...
          $("#ver3").click(function(){
            $("#myModal3").modal('hide');
            window.location='../LISTAR_GG_UNIDAD_TIEMPO.php';
          });
        </script>
    <?php } else { ?>
