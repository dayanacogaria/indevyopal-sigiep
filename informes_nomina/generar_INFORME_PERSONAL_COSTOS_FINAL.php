...
header("Content-Disposition: attachment; filename=Personal_Costos_Nomina.xls");
...
<table width="30%" border="1" cellspacing="0" cellpadding="0">
            ...
            <tr>
                ...
                <td colspan="2"><center><strong>Aprobados</strong></center></td>
                <td colspan="2"><center><strong>Provistos</strong></center></td>
             ...
             <?php
                ...
                $equivalentesPERV="SELECT pc.concepto,
                                          ...
                                          pc.consecutivo,
                                          pc.denominacion,
                                          ca.numero_plazas AS Aprobados,
                                          ca.numero_plazas AS Provistos,
                                          pc.tipo_vinculacion,
                                          Sum(pc.asig_basica),
                                          Sum(pc.gastos_representacion),
                                          Sum(pc.primats),
                                          Sum(pc.prima_gestion),
                                          Sum(pc.prima_localizacion),
                                          Sum(pc.prima_coordinacion),
                                          Sum(pc.prima_riesgo),
                                          Sum(pc.prima_extraordinaria),
                                          Sum(pc.prima_altomando),
                                          Sum(pc.prima_sub_alimentacion),
                                          Sum(pc.auxilio_transporte),
                                          Sum(pc.prima_antiguedad),
                                          Sum(pc.prima_servicios),
                                          Sum(pc.prima_navidad),
                                          Sum(pc.bon_servicios),
                                          Sum(pc.bon_recreacion),
                                          Sum(pc.prima_vacaciones),
                                          Sum(pc.prima_actividad),
                                          Sum(pc.otras_primas),
                                          Sum(pc.cesantias),
                                          Sum(pc.intereses_cesantias)
                                          FROM   gn_personal_costos pc
                                          LEFT JOIN gf_cargo ca ON ca.id_unico=pc.id_cargo
                                          GROUP  BY pc.denominacion";
...
<?php }elseif ($formato==2) {
            ...
            header("Content-Disposition: attachment; filename=Personal_Costos_Nomina.csv");
              ini_set('max_execution_time', 0);
              echo $html;
             ...
             $equivalentesPER = $mysqli->query($equivalentesPERV);
              ...
              while ($filasEq = mysqli_fetch_row($equivalentesPER)) {
               ...
               $html .= "\n";
                ...
                header("Content-Disposition: attachment; filename=Personal_Costos_Nomina.txt");
                ini_set('max_execution_time', 0);
                echo $html;
}
             ?>