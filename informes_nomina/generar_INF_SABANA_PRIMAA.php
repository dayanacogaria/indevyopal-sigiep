...
if($_GET['t']==1){
    ...
    { 
        ...
        { 
            ...
            $this->Ln(4);
            $this->SetX(25);
            $this->Cell(320, 5, utf8_decode('SÁBANA DE LIQUIDACIÓN PRIMA ANTIGUEDAD'), 0, 0, 'C');
            $this->Ln(4); 
            $this->SetFont('Arial', 'B', 8);
            $this->SetX(25);
    ...
    if(empty($_REQUEST['sltUnidadE'])){        
        ...
        $pdf->Cell(15,5, utf8_decode('F. Ingreso'),0,0,'C');
        ...
        for ($e = 0; $e < count($rowe); $e++) { 
            ...
            #*** FI ****#
            $fi= $con->Listar("SELECT DATE_FORMAT(fechaacto, '%d/%m/%Y') 
                FROM gn_vinculacion_retiro 
                WHERE empleado = ".$rowe[$e][0]." 
                AND estado = 1 ORDER BY fechaacto DESC LIMIT 1");
            $pdf->Cellfitscale(15,8, utf8_decode($fi[0][0]),1,0,'R');
            ...
            for ($c = 0; $c < count($rowcn); $c++) {
                ...
                $pdf->Cellfitscale($filas,8, number_format($valor, 2, '.',','),1,0,'R');  
            }        
            $pdf->Ln(8);
        }
        $pdf->SetX(35);
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(75,8, utf8_decode('Total:'),1,0,'C');
       ...
       for ($c = 0; $c < count($rowcn); $c++) {
        ...
        $pdf->Cell(15,5, utf8_decode('Cédula'),0,0,'C');
        $pdf->Cell(45,5, utf8_decode('Nombre'),0,0,'C');
        $pdf->Cell(15,5, utf8_decode('F. Inicio'),0,0,'C');
        $pdf->Cell(15,5, utf8_decode('Básico'),0,0,'C');
        $h2 = 0;
        $h  = 0;
        $alto = 0;
    ...
    WHERE td.nombre = 'Sabana Liquidacion Final'
    ...
    ob_end_clean();
    $pdf->Output(0,'Sabana_LiquidacionF('.date('d/m/Y').').pdf',0);     
}
#******** Tipo Excel *************#
 else{
   ...
   if(empty($_REQUEST['sltUnidadE'])){
        ...
        echo '<br/>&nbsp;';
        echo '<br/>SÁBANA DE LIQUIDACIÓN PRIMA ANTIGUEDAD';
        echo '<br/>&nbsp;';
        ...
        echo '<br/>'.$nombreIdent.': '.$numeroIdent;
        echo '<br/>&nbsp;';
        echo '<br/>SÁBANA DE LIQUIDACIÓN FINAL';
        echo '<br/>&nbsp;';
