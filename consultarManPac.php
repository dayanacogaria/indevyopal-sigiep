<?php
    ...
    if($id == 0){
        ...
        $sql = "SELECT codi_presupuesto FROM gf_rubro_pptal
                WHERE id_unico = $id";
        $a = $mysqli->query($sql);
        ...
        if ($d == 1) {
            $query = "SELECT PADRE.manpac FROM gf_rubro_pptal PADRE 
            ...
            $query1 = "SELECT PADRE.manpac FROM gf_rubro_pptal PADRE 
                    LEFT JOIN gf_rubro_pptal HIJO ON HIJO.predecesor = PADRE.id_unico
