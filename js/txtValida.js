 ...
 * 13/03/2017 |  FUNCION VALIDARDES, PARA DESCRIPCIONES
 ####################################*/
//Evento event, tipo de caracteres permitidos, id del input a validar (en caso de ser 'dec'), núemro de decimales permitidos.
function txtValida(elEvento, permitidos, id, n) {
    ...
    switch(permitidos){//Switch en el que se valida que tipo de datos es permitido
        case 'num'://Si el caso es numerico que permita el ingreso
            permitidos = numeros;
            num = 1;
            break;
        case 'dec'://Si el caso es decimal
            permitidos = numeros;
            num = 2;
            break;
        case 'car'://Si el caso es caracteres
            permitidos = caracteres;
            break;
        case 'num_car'://Si el caso es numeros y caracyeres
    ...
    var evento = elEvento || window.event;//Toma el evento de la ventana
    var codigoCaracter = evento.charCode || evento.keyCode;//Toma el caracter en el evento
    var caracter = String.fromCharCode(codigoCaracter);//Convierte a String el caracter enviado
    var tecla_especial = false;//Validaciones obligatorias para todos los input.
    ...
    if (tecla == 8 || tecla == 0) // Permite retoseso (8) y teclas especiales.
        ...
        return false;
    if(num == 0){//Validación para ingreso de valores con letras.
        ...
        for(var i in teclas_especiales_numeros){//Validación para ingreso de valores númericos
        ...
        //la contidad de decimales despues del punto por ejemplo 16,3
        //Nuevo Ferney
    ...
    if(permt == 0){//Validación para ingreso de caracteres
