...
$cuentas = "SELECT id_unico, codi FROM gf_plan_inventario WHERE compania = $compania ORDER BY codi ASC";
...
if(mysqli_num_rows($cuentas)>0){
    while ($row = mysqli_fetch_array($cuentas)) {
        ...
        for($i = 0;$i <=$cant;$i++){
            ...
            if (mysqli_num_rows($query)>0) {
                ...
                $predecesor = $pred[0];
                $update = "UPDATE gf_plan_inventario SET predecesor ='$predecesor' WHERE id_unico = '$row[0]'";
    ...
    $mensaje= $n." Elementos Actualizadas";
} else {
