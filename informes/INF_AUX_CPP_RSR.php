...
$clp = "SELECT nombre FROM gf_tipo_clase_pptal
        WHERE id_unico=$clasePptal";
$clpR = $mysqli->query($clp);
$claseP = mysqli_fetch_row($clpR);
$clasePp = $claseP[0];
...
if(empty($_REQUEST['fuente'])){
    ...
    AND r.tipoclase = $clasePptal AND r.parametrizacionanno = $para";
} else {
...
elseif($c==2){
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0">
        <tr>
            <th colspan="<?php echo $filas?>" align="center"><strong>
                <br/>&nbsp;
                <br/><?php echo $razonsocial ?>
                <br/><?php echo $nombreIdent.' : '.$numeroIdent."<br/>".$direccinTer.' Tel:'.$telefonoTer ?>
               <br/>&nbsp;
               <br/>LIBRO AUXILIAR PRESUPUESTAL<br/>&nbsp;
               <?php echo ucwords(mb_strtolower($clasePp))?>
    ...
    while ($row = mysqli_fetch_row($cng)) {
        ...
        if(mysqli_num_rows($dr)>0) { 
            ...
            ?>
            <tr>
                <td colspan="<?php echo $filas?>"><strong><?php echo  $row[1].'   '.ucwords($row[2]);?></strong></td>
            </tr>
            ...
            <tr>
                ...
                <td><strong></strong></td>
                <td><strong><?php echo number_format($vaprop-$vpag, 2,  '.', ',');?></strong></td>
                <?php  
                     if ($clasePptal==16) { ?>
                              <td><strong><?php echo number_format($vobl-$vaprop, 2,  '.', ',');?></strong></td>
                     <?php  
                     } 
                ?>
                <td><strong><?php echo number_format($vpag-$vaprop, 2,  '.', ',');?></strong></td>
            </tr>
            <?php 
