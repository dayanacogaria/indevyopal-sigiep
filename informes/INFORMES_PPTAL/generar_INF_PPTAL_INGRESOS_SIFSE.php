...
$select ="SELECT DISTINCT
          ...
          WHERE 
            rpp.codi_presupuesto BETWEEN '$codigoI' AND '$codigoF' 
...
{ 
    ...
    $this->SetFont('Arial','',8);
    $this->Cell(330,5,utf8_decode('REPORTE INGRESOS PRESUPUESTALES'),0,0,'C');
    $this->SetFont('Arial','B',8);
    ...
    $this->Cell(60,7,utf8_decode('CODIGO'),0,0,'C');#
    $this->Cell(45,9,utf8_decode('AÑO'),1,0,'C');#
    $this->Cell(45,7,utf8_decode('TRIMESTRE'),0,0,'C');#
    $this->Cell(45,7,utf8_decode('FUENTE DE '),0,0,'C');#
    $this->Cell(45,7,utf8_decode('PRESUPUESTO '),0,0,'C');#
    $this->Cell(45,7,utf8_decode('PRESUPUESTO '),0,0,'C');#
    $this->Cell(45,7,utf8_decode('MONTO '),0,0,'C');#
    ...
    $this->Cell(60,5,utf8_decode('ESTABLECIMIENTO'),0,0,'C');#
    $this->Cell(45,5,utf8_decode(''),0,0,'C');#
    $this->Cell(45,5,utf8_decode(''),0,0,'C');#
    $this->Cell(45,5,utf8_decode('INGRESOS'),0,0,'C');#
    $this->Cell(45,5,utf8_decode('INICIAL'),0,0,'C');
    $this->Cell(45,5,utf8_decode('DEFINITIVO'),0,0,'C');
    $this->Cell(45,5,utf8_decode('RECAUDOS'),0,0,'C');
...
while ($filactas = mysqli_fetch_array($conejc)) 
{
    ...
    if($p1==0 && $p2==0 && $p3==0){
        ...
        $pdf->Cell(60,4,utf8_decode($codDane),0,0,'C');
        $pdf->Cell(45,4,utf8_decode($anno),0,0,'C');
        $pdf->Cell(45,4,utf8_decode($trim),0,0,'C');
        $pdf->Cell(45,4,utf8_decode($filactas['equivF']),0,0,'C');
        $pdf->Cell(45,4,number_format($p1,2,'.',','),0,0,'R');
        $pdf->Cell(45,4,number_format($p2,2,'.',','),0,0,'R');
        $pdf->Cell(45,4,number_format($p3,2,'.',','),0,0,'R');
        $total1=$total1+$p1;
        $total2=$total2+$p2;
        $total3=$total3+$p3;
...
## TOTALES ##
        ...
        $pdf->CellFitScale(45,4,number_format($total1,2,'.',','),0,0,'R');
        $pdf->CellFitScale(45,4,number_format($total2,2,'.',','),0,0,'R');
        $pdf->CellFitScale(45,4,number_format($total3,2,'.',','),0,0,'R');
 ...
 $res = "SELECT rd.tercero, tr.nombre FROM gf_responsable_documento rd 
        ...
        WHERE td.nombre ='PP15' AND td.compania = $compania ";
...
while (ob_get_length()) {
 ...
 ######### FIN FIRMAS #########
 $pdf->Output(0,utf8_decode('Reporte Ingresos Presupuestales SIFSE('.date('d-m-Y').').pdf'),0);
?>