...
{ 
    ...
    $this->SetFont('Arial','',8);
    //$this->Cell(105,8,'',0);
    $this->Cell(330,5,utf8_decode('AUXILIAR COMPROBANTE PRESUPUESTAL - INGRESOS'),0,0,'C');
    $this->SetFont('Arial','B',8);
    $this->SetX(10);
    ...
    $this->Cell(80,9,utf8_decode('Nombre del Tercero'),0,0,'C');
    $this->Cell(70,9,utf8_decode('Descripción'),0,0,'C');
    $this->Cell(33,9,utf8_decode('Valor'),0,0,'C');
    $this->Cell(33,9,utf8_decode('Modificaciones'),0,0,'C');
    $this->Cell(33,9,utf8_decode('Total'),0,0,'C');
...
while ($rubro = mysqli_fetch_assoc($rubroP)){
    ...
    while ($row1 = mysqli_fetch_array($con1)) {
       ...
       if($numd>0) {
        ...
        $pdf->Cell(50,5,utf8_decode('Rubro: '.$rubro['codigoR'].' - '.$rubro['nombreR']));
        $pdf->Ln(5);
        $pdf->SetFont('Arial','',8);
	...
	$af = $mysqli->query($a);
       ...
       if($numpaginas>$paginactual){
        ...
        $pdf->CellFitScale(33,4,number_format($p2,2,'.',','),0,0,'R');
        $pdf->CellFitScale(33,4,number_format($p3,2,'.',','),0,0,'R');
        $alt = max($h,$h1);
        $pdf->Ln($alt);
        $total1 = $p3+$total1;
        $total2 +=$p3;
...
$pdf->Cell(244,4,utf8_decode('TOTAL RUBRO : '.$rubro['codigoR'].' - '.$rubro['nombreR']),0,0,'R');
$pdf->Cell(66,4,number_format($total2,2,'.',','),0,0,'R');
$pdf->Ln(5);
}
}
## TOTALES ##
...
#########  Fin Estructura Funcionamiento   ############
 ...
 $pdf->Output(0,'Informe_Auxiliares_Presupuestales_Ingresos ('.date('d/m/Y').').pdf',0);
?>