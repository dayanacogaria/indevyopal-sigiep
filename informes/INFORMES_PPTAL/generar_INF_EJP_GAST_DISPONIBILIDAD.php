...
$ctas = "SELECT DISTINCT
         ...
         WHERE rpp.codi_presupuesto BETWEEN '$codigoI' AND '$codigoF' AND rpp.tipoclase=7 AND rpp.tipovigencia=1 "
...
{ 
    ...
    $this->Cell(25,9,utf8_decode('FUENTE'),1,0,'C');################
...
$sql2 = "SELECT DISTINCT 
                        tmp.cod_rubro           as codrub, 
                        tmp.nombre_rubro        as nomrub,
                        f.nombre                as codfte,
                        tmp.ptto_inicial        as ppti,
                        tmp.adicion             as adi,
                        tmp.reduccion           as red,
                        tmp.tras_credito        as tcred,
                        tmp.tras_cont           as trcont,
                        tmp.presupuesto_dfvo    as ppdf,
                        tmp.disponibilidades    as disp,
                        tmp.saldo_disponible    as sald,
                        tmp.registros           as reg,
                        tmp.registros_abiertos  as rega,
                        tmp.total_obligaciones  as tobl,
                        tmp.total_pagos         as tpag,
                        tmp.reservas            as reserv,
                        tmp.cuentas_x_pagar     as cpag                        
           from temporal_consulta_pptal_gastos  tmp
           LEFT JOIN gf_fuente f ON f.id_unico= tmp.cod_fuente 
            ORDER BY tmp.cod_rubro ASC";
...
while ($filactas = mysqli_fetch_array($conejc)) 
{
    ...
    if ($p1 == 0  && $p2 == 0  && $p3 == 0 && $p4==0 && $p5==0 && $p6==0 && $p7==0 && $p8==0 && $p9==0 && $p10==0 && $p11==0 && $p12==0 && $p13==0)
        ...
        $y = $pdf->GetY();
        $x = $pdf->GetX();        
        $pdf->MultiCell(60,4,utf8_decode($filactas['nomrub']),1,'L');        
        $y2 = $pdf->GetY();
        $h = $y2-$y;
        $px = $x + 60;
        $pdf->SetXY($px, $y);
        ...
        if($filactas['codfte']!=''){
            $pdf->CellFitScale(25,4,utf8_decode($filactas['codfte']),1,0,'L');
        }
        else{
            $pdf->Cell(25,4,utf8_decode($filactas['codfte']),1,0,'L');
        }
...
$pdf->Ln(30);
 ...
 $res = "SELECT   c.nombre,rd.orden,IF(CONCAT_WS(' ',
        ...
        //$pdf->CellFitScale(12,9,$res,1,0,'C');
