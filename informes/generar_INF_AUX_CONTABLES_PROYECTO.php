...
# 06/03/2017 |ERICA G | ARREGLO CONSULTAS FECHA COMPROBNTE 
# 03/03/2017 |Erica G //Arreglo de consultas
# 12:00 | 11/02/2017 | Erica G //Arreglo de consultas
# 03:50 | 10/02/2017 | Erica G
# 10:40 | 10/02/2017 | Jhon Numpaque
# Se incluyo salto de pagina para tercero y descripción    
...
{ 
    ...
    global $numpaginas;
    global $ruta;
    global $proyectoInicial;
    global $proyectoFinal;
    ...
    $this->SetFont('Arial','',8);
    $this->SetX(25);
    $this->Cell(315,5,utf8_decode('AUXILIAR CONTABLE'),0,0,'C');
    $this->SetFont('Arial','B',8);
    $this->SetX(25);
    ...
    $this->SetFont('Arial','',7);
    $this->SetX(25);
    $this->Cell(315,5,utf8_decode('Proyecto Inicial: '.strtoupper($proyectoInicial).' - Proyecto Final: '.strtoupper($proyectoFinal)),0,0,'C');
...
$cnt = 0;
    $cuentas = "SELECT DISTINCT cuenta FROM gf_detalle_comprobante dc 
            ...
            WHERE cn.parametrizacionanno = $parmanno AND dc.valor IS NOT NULL AND c.codi_cuenta BETWEEN '$cuentaMin' AND '$cuentaMax' 
...
$fechafin1 =$anio12.'/'.$mes12.'/'.$dia12;     
 ...
 $sql = "SELECT DISTINCT
                                        ...
                                        AND cn.parametrizacionanno = $parmanno AND cta.parametrizacionanno = $parmanno 
...
$pdf->SetFont('Arial','B',7);
   $pdf->SetX(20);
   $pdf->Cell(165,4,utf8_decode($proyectocnt.' - Código Cuenta: '.$codicuenta),0);
   $pdf->Cell(20,4,utf8_decode('Saldo Inicial: '),0);
   $pdf->Cell(20,4,utf8_decode('$'.number_format($saldoTA,2,'.',',')),0);
   $pdf->Cell(205,4,'',0);
   $pdf->Ln(4);    
$pdf->SetFont('Arial','',7);
...
while ($fila = mysqli_fetch_array($cp)) 
    ...
    $deb=0;
     ...
     $cr = 0;   
        ...
        $y1 = $pdf->GetY();
        $x1 = $pdf->GetX();
        $pdf->MultiCell(65,4,utf8_decode(ucwords(mb_strtolower($fila['NOMBRE'] )).' - '.$fila['trnum']),0,'L');
        $y2 = $pdf->GetY();
        $h = $y2-$y1;
        $px = $x1+65;
...
$pdf->Output(0,'Informe_Auxiliares_Contables ('.date('d/m/Y').').pdf',0);
?>