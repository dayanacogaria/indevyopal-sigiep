...
if($_GET['t']==1){
    ...
    { 
        ...
        { 
            ...
            $this->Ln(4);
            $this->SetX(25);
            $this->Cell(320, 5, utf8_decode('SÁBANA RETROACTIVO'), 0, 0, 'C');
            $this->Ln(4); 
            $this->SetFont('Arial', 'B', 8);
            $this->SetX(25);
    ...
    if(!empty($unidad) && !empty($grupog)){        
        ...
        $pdf->Cell(66,5, utf8_decode('Nombre'),0,0,'C');
        ...
        $pdf->Cell(66,$alto, utf8_decode(''),1,0,'C');
        ...
        for ($e = 0; $e < count($rowe); $e++) {
            ...
            $pdf->Cellfitscale(66,8, utf8_decode($rowe[$e][5]),1,0,'L');
        ...
        $pdf->Cell(91,8, utf8_decode('Total:'),1,0,'C');
       ...
       for ($c = 0; $c < count($rowcn); $c++) {
        ...
        $rowg = $con->Listar("SELECT DISTINCT e.grupogestion, gg.nombre 
            ...
            LEFT JOIN gn_empleado e  ON n.empleado = e.id_unico 
            LEFT JOIN gn_grupo_gestion gg ON e.grupogestion = gg.id_unico 
            WHERE gg.id_unico=$grupog AND n.periodo = $periodo");
        for ($g = 0; $g < count($rowg); $g++) {
            $pdf->Cell(50,5, utf8_decode('GRUPO DE GESTIÓN:'),0,0,'L');
            $pdf->Cell(100,5, utf8_decode($rowg[$g][1]),0,0,'L');
            $pdf->Ln(4);
        ...
        for ($u = 0; $u < count($rowu); $u++) {
            ...
            $rowg = $con->Listar("SELECT DISTINCT e.grupogestion, gg.nombre 
                ...
                LEFT JOIN gn_empleado e ON n.empleado = e.id_unico 
                LEFT JOIN gn_grupo_gestion gg ON e.grupogestion = gg.id_unico 
                WHERE e.unidadejecutora = $unidad AND n.periodo = $periodo");
            $ttg = count($rowg);
            for ($g = 0; $g < count($rowg); $g++) {
    ...
    ob_end_clean();$pdf->Output(0,'Sabana_Nomina('.date('d/m/Y').').pdf',0);     
}
#******** Tipo Excel *************#
 else{
    ...
    if(!empty($unidad) && !empty($grupog)){        
        ...
        echo '<td colspan="2"><strong>Firma</strong></td>';
        echo '</tr>';
