...
#+ Datos  
$rowf = $con->Listar("SELECT DISTINCT fa.id_unico, DATE_FORMAT(fa.fecha_ven, '%d/%m/%Y'), 
...
LEFT JOIN ga_detalle_factura df ON fa.id_unico = df.factura 
...
$pdf->Ln(15);
$pdf->SetFont('Arial','B',10);
$pdf->Cell(190,5,utf8_decode('DATOS FACTURA'),1,0,'C');
$pdf->Ln(5);
$cx = $pdf->GetX();
$cy = $pdf->GetY();
...
$pdf->SetFont('Arial','B',10);
$pdf->Cell(170,5,utf8_decode('TOTAL FACTURA'),1,0,'R');
$pdf->Cell(20,5,utf8_decode(number_format($tf, 2, '.', ',')),1,0,'R');
$pdf->Ln(5);
ob_end_clean();		
$pdf->Output(0,'Factura_Acuerdo.pdf',0);
