...
<title>Auxiliares Contables Tercero</title>
</head>
<body>
...
<style>
    label #sltTci-error, #sltTcf-error, #fechaini-error, #fechafin-error, #sltctai-error, #sltctaf-error, #sltTeri-error, #sltTerf-error  {
    display: block;
    color: #155180;
    font-weight: normal;
    font-style: italic;
...
<div class="container-fluid text-center">
    ...
    <?php require_once 'menu.php'; ?>
        <div class="col-sm-8 text-left" style="margin-left: -16px;margin-top: -20px"> 
            <h2 align="center" class="tituloform">Auxiliares Contables Tercero</h2>
            <div style="border: 4px solid #020324; border-radius: 10px; margin-left: 4px; margin-right: 4px;" class="client-form">
                ...
                <?php 
                    $cuentaI = "SELECT id_unico,CONCAT(codi_cuenta,' - ',nombre) "
            ...
            <?php 
                $cuentaF = "SELECT id_unico, codi_cuenta, nombre "
                        . "from gf_cuenta WHERE parametrizacionanno = $anno ORDER BY codi_cuenta DESC";
            ...
            <?php
                $terceroI="SELECT IF(CONCAT_WS(' ',
                  ...
                  CONCAT_WS(' ',
                    ...
                    ORDER BY ter.numeroidentificacion ASC ";
                $terceroI = $mysqli->query($terceroI);        
            ?>
            <div class="form-group" style="margin-top: -5px">
                <label for="sltTeri" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Tercero Inicial:</label>
                <select name="sltTeri" id="sltTeri" style="height: auto" class=" select2_single form-control" title=
                        "Seleccione Tercero Inicial" required>
                    <option value="">Tercero Inicial</option>
                <?php 
                    while ($terI = mysqli_fetch_row($terceroI)) 
                { 
                ?>
                    <option value="<?php echo $terI[1];?>"><?php echo mb_strtoupper($terI[1]).' - '.ucwords(mb_strtolower($terI[0]));?></option>                                
                <?php 
                }
                 ?>                                    
                </select>
           ...
           <?php
                $terceroF="SELECT IF(CONCAT_WS(' ',
                  ...
                  CONCAT_WS(' ',
                    ...
                    ter.apellidouno,
                    ter.apellidodos)) AS NOMBRE, ter.numeroidentificacion 
                    FROM gf_tercero ter  WHERE ter.compania = $compania  ORDER BY ter.numeroidentificacion DESC ";
                $terceroF = $mysqli->query($terceroF);        
            ?>
            <div class="form-group" style="margin-top: 0px">
                <label for="sltTerf" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Tercero Final:</label>
                <select name="sltTerf" id="sltTerf" style="height: auto" class=" select2_single form-control" title=
                        "Seleccione Tercero Final" required>
                    <option value="">Tercero Final</option>
                <?php 
                    while ($terF = mysqli_fetch_row($terceroF)) 
                { 
                ?>
                    <option value="<?php echo $terF[1];?>"><?php echo mb_strtoupper($terF[1]).' - '.ucwords(mb_strtolower($terF[0]));?></option>                                
                <?php 
                }
                 ?>                                    
                </select>
            ...
            <div class="form-group" style="margin-top: -5px">
                <label for="sltTipoI" class="col-sm-5 control-label">Informe Por:</label>
                <select name="sltTipoI" id="sltTipoI" class="form-control select2_single" title="Informe por">
                    <option value="0">Informe</option>                    
                    <option value="1">Auxiliar para Retenciones</option>                    
                </select>
            </div>
...
function reporteExcel(){
    var opcion = document.getElementById('sltTipoI').value;
    switch(opcion){
        case ('0'):
            $('form').attr('action', 'informes/generar_INF_AUX_CNT_TER_EXCEL.php');
        break;
        case ('1'):
            $('form').attr('action', 'informes/generar_INF_AUX_RET.php?window=excel');
        break;
    }   
}
...
function reportePdf(){
    ...
    switch(opcion){
        case ('0'):
            $('form').attr('action', 'informes/generar_INF_AUX_CONTABLE_TERCERO.php');
        break;
        case ('1'):
            $('form').attr('action', 'informes/generar_INF_AUX_RET.php?window=pdf');
        break;
    }
}
