...
if(empty($_GET['id'])) {
    ...
    tf.id_unico, tf.codigo, tf.nombre, 
    ...
    FROM gf_resolucion rf
    LEFT JOIN gf_tipo_comprobante_pptal tf ON rf.tipo_comprobante_pptal = tf.id_unico 
    WHERE tf.compania = $compania ");
} elseif(($_GET['id'])==1) {
    $titulo = "Registrar ";
    $titulo2= ".";
    $rtc    = $con->Listar("SELECT id_unico, codigo, nombre FROM gf_tipo_comprobante_pptal WHERE compania = $compania ");
} elseif(($_GET['id'])==2) {
    $titulo = "Modificar ";
    $id     = $_GET['id_r'];
    $row    = $con->Listar("SELECT rf.id_unico,
    ...
    $rtc    = $con->Listar("SELECT id_unico, codigo, nombre FROM gf_tipo_comprobante_pptal WHERE compania = $compania AND id_unico != ".$row[0][1]);
}
?>
...
<body>
    <div class="container-fluid text-center">
        <div class="row content">    
            ...
            <div class="col-sm-10 text-left" style="margin-top:-20px">
                <h2 id="forma-titulo3" align="center" style=" margin-right: 4px; margin-left: 4px;"><?php echo $titulo.' Resolución Comprobantes'?></h2>
                ...
                <div class="table-responsive" style="margin-left: 5px; margin-right: 5px;">
                    <div class="table-responsive" style="margin-left: 5px; margin-right: 5px;">
                        <table id="tabla" class="table table-striped table-condensed" class="display" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <td width="30px"></td>
                                    <td><strong>Tipo Comprobante Pptal</strong></td>
                                ...
                                <?php for ($i = 0; $i < count($row); $i++) { ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="GF_RESOLUCION.php?id=2&id_r=<?php echo md5($row[$i][0]); ?>"><i title="Modificar" class="glyphicon glyphicon-edit" ></i></a>
                                        </td>
                        ...
                        <div align="right"><a href="GF_RESOLUCION.php?id=1" class="btn btn-primary" style="box-shadow: 0px 2px 5px 1px gray;color: #fff; border-color: #1075C1; margin-top: 20px; margin-bottom: 20px; margin-left:-20px; margin-right:4px">Registrar Nuevo</a> </div>       
                    </div>
                </div>
                <?php }  elseif(($_GET['id'])==1){ ?>
                    <a href="gf_resolucion.php" class="glyphicon glyphicon-circle-arrow-left" style="display:inline-block;margin-left:10px; font-size:150%; vertical-align:middle;text-decoration:none" title="Volver"></a>
                        ...
                        <form name="form" id="form" class="form-horizontal" method="POST"  enctype="multipart/form-data" action="javascript:registrar()">
                            ...
                            <div class="form-group">
                                <div class="form-group form-inline  col-md-5 col-lg-5" text-aling="left">
                                    <label for="tipo" class="col-sm-12 control-label"><strong style="color:#03C1FB;">*</strong>Tipo Comprobante Pptal:</label>
                                </div>
                                <div class="form-group form-inline  col-md-3 col-lg-3" style="margin-left:10px">
                                    <select name="tipo" id="tipo" class="form-control select2" title="Seleccione Tipo Comprobante Pptal"  required="required">
                                            <option value="">Tipo Comprobante Pptal</option>
    ...
    <script>
        function registrar(){
            ...
            $.ajax({
                type: 'POST',
                url: "jsonPptal/gf_comprobantesJson.php?action=1",
                data:formData,
                contentType: false,
                processData: false,
                success: function(response)
                {
                    ...
                    if(response==1){
                        ...
                        $("#Aceptar").click(function(){
                            document.location='GF_RESOLUCION.php';
                        })
                    } else {
    ...
    <script>
        function modificar(){
            ...
            $.ajax({
                type: 'POST',
                url: "jsonPptal/gf_comprobantesJson.php?action=2",
                data:formData,
                contentType: false,
                processData: false,
                success: function(response)
