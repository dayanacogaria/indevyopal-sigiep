...
<html>
    <head>
        <title>Configuración Interfaz Reteica</title>
    ...
    <body> 
        <div class="container-fluid text-center">
            <div class="row content">
                ...
                <div class="col-sm-10 text-left">
                    <h2 align="center" class="tituloform" style="margin-top:-3px">Configuración Interfaz Reteica</h2>
                            ...
                            <div class="form-group" style="margin-top: -5px">
                                ...
                                <select name="vigencia" id="vigencia" class="select2_single form-control" title="Seleccione Vigencia" style="height: auto " required>
                                    ...
                                    if(empty($_GET['vg'])) { 
                                        ...
                                        $vg = $con->Listar("SELECT id_unico, nombre, valor FROM gf_vigencias_interfaz_reteica WHERE parametrizacionanno = $anno");
                                        for ($i = 0; $i < count($vg); $i++) {
                                           echo '<option value="'.$vg[$i][0].'">'.$vg[$i][1].' - '.$vg[$i][2].'</option>'; 
                                        }
                                    } else {
                                        $vg1 = $con->Listar("SELECT id_unico, nombre, valor FROM gf_vigencias_interfaz_reteica WHERE id_unico =".$_GET['vg']);
                                        echo '<option value="'.$vg1[0][0].'">'.$vg1[0][1].' - '.$vg1[0][2].'</option>';
                                        $vg = $con->Listar("SELECT id_unico, nombre, valor FROM gf_vigencias_interfaz_reteica  WHERE parametrizacionanno = $anno AND id_unico !=".$_GET['vg']);
                                        for ($i = 0; $i < count($vg); $i++) {
                                           echo '<option value="'.$vg[$i][0].'">'.$vg[$i][1].' - '.$vg[$i][2].'</option>'; 
                                        }
                                    }
                                ...
                                <tbody>
                                    <?php if(!empty($_GET['vg'])) { 
                                        ...
                                        $conc = $con->Listar("SELECT * FROM gc_concepto_comercial 
                                            WHERE (tipo_ope=2 OR tipo_ope=3) AND ((clase=2) OR (clase=1 AND tipo=4)) ORDER BY codigo ASC ");
                                        ...
                                        for ($j = 0; $j < count($conc); $j++) {
                                            ...
                                            $cf = $con->Listar("SELECT cf.id_unico, 
                                                    ...
                                                    WHERE vg.parametrizacionanno = $anno AND cf.vigencia_ica = $vigencia 
        ...
        <script>
            $("#vigencia").change(function(){
                ...
                if(vigencia!=""){
                    document.location ='GF_CONFIGURACION_RETEICA.php?vg='+vigencia;
                }
            })
        </script>
        ...
        <script>
            function guardar(id){
               ...
               $.ajax({
                type: 'POST',
                url: "jsonPptal/gf_interfaz_ComercioJson.php?action=15",
                data:formData,
                contentType: false,
                processData: false,
        ...
        <script>
            function open_modal_r(id) {  
               ...
               var form_data={                            
                 ...
                 $.ajax({
                    type: 'POST',
                    url: "GF_CONFIGURACION_RETEICA_MODAL.php#mdlModificar",
                    data:form_data,
        ...
        <?php require_once './GF_CONFIGURACION_RETEICA_MODAL.php'; ?>
    </body>
</html>
