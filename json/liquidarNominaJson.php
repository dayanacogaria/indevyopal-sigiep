...
$parmUvt = "SELECT uvt FROM gf_parametrizacion_anno WHERE id_unico = $anno";
$rparmUvt   = $mysqli->query($parmUvt);
$uvtF    = mysqli_fetch_row($rparmUvt);

$valorUvt=$uvtF[0];

if ($valorUvt==NULL || $valorUvt==0) {
    $resUvt=1;
}else{
    $resUvt=2;
}
...
$resultaVa = $mysqli->query($sql3);
 $datos = array();
 while($EMPV = mysqli_fetch_row($resultaVa)){

 #Tipo Empleado y Categoria= Obligatorio. 
       $sqlTipEm="SELECT  
                 ...
                 WHERE e.id_unico = $EMPV[0]";
       $resTipE = $mysqli->query($sqlTipEm);
       $rowTipEm = mysqli_fetch_row($resTipE);
        $id_tipoe=$rowTipEm[0];
        if ($id_tipoe!=null) {
               $sqlCate="SELECT  
                       ca.id_unico,
               ...
               LEFT JOIN   gn_categoria ca         ON ca.id_unico=tc.categoria
               WHERE e.id_unico = $EMPV[0]";
               $resCat = $mysqli->query($sqlCate);
               //$rowCat = mysqli_num_rows($resCat); 
               $rowCatV = mysqli_fetch_row($resCat); 
               $id_cate=$rowCatV[0];
                       if($id_cate==null){
                           $respuestaP=2;  
                           $mensaje=4;
    ...
    $sqlParam = "SELECT asaludemple, apensionemple FROM gn_parametros_liquidacion WHERE vigencia = $anno and tipo_empleado  = ".$EMPV[11];
    $resPar = $mysqli->query($sqlParam);
    $rowPar = mysqli_fetch_row($resPar);
if ($respuestaP!=2) {
    if($rowPar[0]!=0){
         ...
         $sqlValSalud="SELECT  
                          ...
                          WHERE a.empleado = $EMPV[0]
                          AND ta.id_unico=1";
          $resValSalud = $mysqli->query($sqlValSalud);
          $rowValSalud = mysqli_num_rows($resValSalud);
              if ($rowValSalud>0) {
                  $respuestaP=1;
              }else{
                $respuestaP=2;
 ...
 if ($respuestaP!=2) {   
    if($rowPar[1]!=0){
        $sqlValPension="SELECT  
                    ...
                    WHERE a.empleado = $EMPV[0]
                    AND ta.id_unico=2";
         $resValPension = $mysqli->query($sqlValPension);
         $rowValPension = mysqli_num_rows($resValPension);
              if ($rowValPension>0) {
                  $respuestaP=1;
              }else{
                $respuestaP=2;
     ...
     if($respuestaP==2){
      ...
      if ($mensaje==1) {
           $msg='El empleado '.$EMPV[2].' tenga fondo de Pension relacionado.<BR>';
      }else if($mensaje==2){
           $msg='El empleado '.$EMPV[2].' tenga fondo de Salud relacionado.<BR>';
      }else if($mensaje==3){
           $msg='El empleado '.$EMPV[2].' tenga Empleado Tipo relacionado.<BR>';
      }else if($mensaje==4){
           $msg='El empleado '.$EMPV[2].' tenga Categoria relacionada.<BR>';
      }
      $datos[] =$msg;
    }
  ...
  $datosC1 = implode(",", $datos);
  $datosC = str_replace(",", "",$datosC1);
  if(!empty($datos)){
      $resM=2;
   }else{
   $resM=1;
...
while($EMP = mysqli_fetch_row($resulta)){
    ...
    $limitbon   = $rowP[43];//Limite Bonificacion Servicios
    ...
    if($nres9 > 0){            
        ...
        $pensionVACE=ROUND($pensionVACE); 
        //$pensionVACE = ceil($pensionVACE/$redondeo)*$redondeo;
       
      
        //echo 'Valordesfa'.$redondeo.'<--';
    ...
    //echo 'salario '.$SE[0].'<br>';
    //echo '*tdt '.$TDT.'<br>';
    //echo '/DTRABA '.$DTR[0].'<br>';
    ...
    if($IBC < $psm ){
        ...
        if (($ggIb[1]>=$DTR[1] && $ggIb[1]<=$DTR[2]) || ($diasv!=0)){
           $IBC=$IBC;
...
<!--Modal para informar al usuario que no se ha podido registrar -->
<div class="modal fade" id="modalValidarData" role="dialog" align="center" >
<div class="modal-dialog">
  <div class="modal-content">
    <div id="forma-modal" class="modal-header">
    ...
    <div class="modal-body" style="margin-top: 8px">
      <p><b>Verifique que</b><br> <?php echo json_encode($datosC);?></p>
    </div>
    <div id="forma-modal" class="modal-footer">
      <button type="button" id="verV" class="btn" style="color: #000; margin-top: 2px" data-dismiss="modal">Aceptar</button>
    </div>
  </div>
</div>
</div>

<!--Modal para informar al usuario el uvt no esta configurado -->
<div class="modal fade" id="modalValidarUvt" role="dialog" align="center" >
<div class="modal-dialog">
  <div class="modal-content">
    <div id="forma-modal" class="modal-header">
    ...
    <div class="modal-body" style="margin-top: 8px">
      <p><b>Verifique que la parametrización del año, en el campo UVT este configurado correctamente.<br>
            Ruta: Archivos Generales/Archivos Basicos/Parametrización Año</p>
    </div>
    <div id="forma-modal" class="modal-footer">
      <button type="button" id="verV1" class="btn" style="color: #000; margin-top: 2px" data-dismiss="modal">Aceptar</button>
    </div>
  </div>
</div>
</div>
...
<?php if($resUvt == 1 ){ ?>
    ...
    <script type="text/javascript">
            $("#modalValidarUvt").modal('show');
            $("#verV1").click(function(){
                $("#modalValidarUvt").modal('hide');      
                 window.history.go(-1);
            });
        </script>
<?php }else{ ?>
<?php if($resM == 2 ){ ?>
        <script type="text/javascript">
            $("#modalValidarData").modal('show');
            $("#verV").click(function(){
                $("#modalValidarData").modal('hide');      
                window.history.go(-1);
            });
        </script>
<?php }else{ ?>
