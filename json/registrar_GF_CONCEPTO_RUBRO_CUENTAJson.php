...
if(empty($_POST["sltCuentaD"])){
  $cuentaDes  = 'NULL';
}else{
  $cuentaDes  = '"'.$mysqli->real_escape_string(''.$_POST["sltCuentaD"].'').'"';
}
...
$num = 0;
if(!empty($_POST['cuentaD']) && empty($_POST['cuentaC']) && !empty($_POST['centroC']) && !empty($_POST['proyecto']) && empty($_POST['sltCuentaI']) && empty($_POST['sltCuentaImpo'])){
  $queryU="SELECT * FROM gf_concepto_rubro_cuenta 
          WHERE concepto_rubro    = $conceptoR 
          AND cuenta_debito       = $cuentaD 
          AND cuenta_credito      = NULL
          AND centrocosto         = $centroC
          AND proyecto            = $proyecto
          AND cuenta_iva          = NULL
          AND cuenta_impoconsumo  = NULL";
  $car = $mysqli->query($queryU);
  $num=mysqli_num_rows($car);
}else if(!empty($_POST['cuentaD']) && !empty($_POST['cuentaC']) && !empty($_POST['centroC']) && !empty($_POST['proyecto']) && !empty($_POST['sltCuentaI']) && !empty($_POST['sltCuentaImpo'])){
  $queryU="SELECT * FROM gf_concepto_rubro_cuenta 
          WHERE concepto_rubro    = $conceptoR 
          AND cuenta_debito       = $cuentaD 
...
if($num == 0) {
   $insert = "INSERT INTO gf_concepto_rubro_cuenta (concepto_rubro, cuenta_debito, "
           ...
           . "cuenta_debito_provision, cuenta_credito_provision, cuenta_descuento , tercero ) "
          . "VALUES($conceptoR, $cuentaD, $cuentaC, $centroC, $proyecto, $cuentaIva, "
           . "$cuentaImpo, $cuentaDebitoSeguro, $cuentaCreditoSeguro, "
           . "$terceroSeguro, $cuentaDebitoProvision, $cuentaCreditoProvision, $cuentaDes, $tercero)";
