<?php
  ...
  $num = 0;
  //Captura de datos e instrucción SQL para su inserción en la tabla gf_tercero_retencion.
  $tercero      = '"'.$mysqli->real_escape_string(''.$_POST['tercero'].'').'"';
  $clase_ret    = '"'.$mysqli->real_escape_string(''.$_POST['sltClaseR'].'').'"';
  $tipo_ret     = '"'.$mysqli->real_escape_string(''.$_POST['sltTipoR'].'').'"';
  $valor        = $_POST['valor']; 
  $observacion  = '"'.$mysqli->real_escape_string(''.$_POST['txtObservacion'].'').'"'; 

 
  $queryU = "SELECT id_unico FROM gf_tercero_retencion WHERE tercero = $tercero";
  ...
  {
    $insertSQL = "INSERT INTO gf_tercero_retencion (tercero, clase_retencion, tipo_retencion,valor,observaciones) VALUES($tercero, $clase_ret, $tipo_ret,$valor,$observacion)";
