...
$ruta       = '"'.$mysqli->real_escape_string(''.$_POST['txtRuta'].'').'"';
$empleado   = '"'.$mysqli->real_escape_string(''.$_POST['sltEmpleado'].'').'"';
$documento  = '"'.$mysqli->real_escape_string(''.$_POST['sltDocumento'].'').'"';
$id         = '"'.$mysqli->real_escape_string(''.$_POST['id'].'').'"';
   
//modificar ne la base de datos
  $insertSQL = "UPDATE gn_documentacion_empleado SET ruta = $ruta, empleado = $empleado, documento = $documento WHERE id_unico = $id";
...
<script type="text/javascript">
  ...
  $("#ver1").click(function(){
    $("#myModal1").modal('hide');
    window.location='../listar_GN_DOCUMENTACION_EMPLEADO.php';
  });
</script>
<?php }else{ ?>
<script type="text/javascript">
