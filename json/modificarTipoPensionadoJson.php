...
//obtiene los datos que se van a modificar
      $tiposp = '"'.$mysqli->real_escape_string(''.$_POST['sltTipoSP'].'').'"';
      $nombre  = '"'.$mysqli->real_escape_string(''.$_POST['txtNombre'].'').'"';
      $id  = '"'.$mysqli->real_escape_string(''.$_POST['id'].'').'"';
   
//modificar ne la base de datos
  $insertSQL = "UPDATE gn_tipo_pensionado SET nombre=$nombre, tiposoipensionado=$tiposp WHERE id_unico = $id";
...
<script type="text/javascript">
  ...
  $("#ver1").click(function(){
    $("#myModal1").modal('hide');
    window.location='../listar_GN_TIPO_PENSIONADO.php';
  });
</script>
<?php }else{ ?>
<script type="text/javascript">
