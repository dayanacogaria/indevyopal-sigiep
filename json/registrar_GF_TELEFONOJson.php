<?php
  ...
  $tipoT  = '"'.$mysqli->real_escape_string(''.$_POST['nombre'].'').'"';
...
//Consulta del ID de la tabla parametrización año.
  // $queryUltimo = "SELECT MIN(id_unico) FROM gf_tercero";
  // $ultimo = $mysqli->query($queryUltimo);
  // $row = mysqli_fetch_row($ultimo);
  // $tercero = $row[0];
///---->> PROVISIONAL <<---------

  $insertSQL = "INSERT INTO gf_telefono (tipo_telefono, valor, tercero) VALUES($tipoT, $valor, $tercero)";
...
<script type="text/javascript">
  ...
  $("#ver1").click(function(){
    $("#myModal1").modal('hide');
    window.location='../GF_TELEFONO_DB.php';
  });
</script>
<?php }else{ ?>
<script type="text/javascript">
