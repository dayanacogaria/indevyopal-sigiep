...
################ MODIFICACIONES ####################
#02/06/2017 | Anderson Alarcon | Modifique consulta Update  
####################################################
...
//obtiene los datos que se van a modificar
    ...
    $indicador  = '"'.$mysqli->real_escape_string(''.$_POST['sltIndicador'].'').'"';
    $tarifa     = '"'.$mysqli->real_escape_string(''.$_POST['sltTarifa'].'').'"';
    $predio     = '"'.$mysqli->real_escape_string(''.$_POST['sltPredio'].'').'"';
    $id  = '"'.$mysqli->real_escape_string(''.$_POST['id'].'').'"'; 
//modificar ne la base de datos
  $insertSQL = "UPDATE gr_avaluo SET valor=$valor, indicador=$indicador, tarifa=$tarifa, predio=$predio WHERE id_unico = $id";
...
<script type="text/javascript">
  ...
  $("#ver1").click(function(){
    $("#myModal1").modal('hide');
    window.location='../listar_GR_AVALUO.php';
  });
</script>
<?php }else{ ?>
<script type="text/javascript">
