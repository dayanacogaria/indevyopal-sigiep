<?php
  ...
  $tipoA  = '"'.$mysqli->real_escape_string(''.$_POST['tipoA'].'').'"'; 
  $tercero = '"'.$mysqli->real_escape_string(''.$_POST['tercero'].'').'"';
  

  $num = 0;
  $queryT="SELECT tipoactividad, tercero FROM gf_tipo_actividad_tercero WHERE  tipoactividad= $tipoA AND tercero=$tercero";
  $sql = $mysqli->query($queryT);
  $num=mysqli_num_rows($sql);


 if($num == 0)//Si no existe el año, se realizará la inserción de datos en la tabla.
  {
  $insertSQL = "INSERT INTO gf_tipo_actividad_tercero (tipoactividad, tercero) VALUES($tipoA, $tercero)";
      ...
      <div class="modal-content">
        ...
        <p><?php
              if($num != 0) //Si se encuentra el año, informará al usuario.
                echo "Tipo Actividad ingresado ya existe.";
              else
...
<script type="text/javascript">
  ...
  $("#ver1").click(function(){
    $("#myModal1").modal('hide');
    window.location='../GF_TIPO_ACTIVIDAD_TERCERO.php';
  });
</script>
<?php }else{ ?>
<script type="text/javascript">
