...
################ MODIFICACIONES ####################
#10/06/2017 | Anderson Alarcon | cambie update en interes descuento
############################################
...
$interes      = '"'.$mysqli->real_escape_string(''.$_POST['sltInteres'].'').'"';
$financiacion = '"'.$mysqli->real_escape_string(''.$_POST['sltFinanciacion'].'').'"';
$tipo         = '"'.$mysqli->real_escape_string(''.$_POST['sltTipo'].'').'"';
$mes          = '"'.$mysqli->real_escape_string(''.$_POST['sltMes'].'').'"';
$id           = '"'.$mysqli->real_escape_string(''.$_POST['id'].'').'"';

$insertSQL = "UPDATE gr_interes_descuento SET anio=$anio, valor=$valor, interes=$interes, financiacion=$financiacion, tipo=$tipo, mes=$mes where id_unico =$id";
...
<script type="text/javascript">
  ...
  $("#ver1").click(function(){
    $("#myModal1").modal('hide');
    window.location='../listar_GR_INTERES_DESCUENTO.php';
  });
</script>
<?php }else{ ?>
<script type="text/javascript">
