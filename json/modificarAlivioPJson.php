...
//obtiene los datos que se van a modificar
    ...
    $numacuerdo  = '"'.$mysqli->real_escape_string(''.$_POST['txtNumAcuerdo'].'').'"';
    $descripcion = '"'.$mysqli->real_escape_string(''.$_POST['txtDescripcion'].'').'"';
    $id  = '"'.$mysqli->real_escape_string(''.$_POST['id'].'').'"'; 
//modificar ne la base de datos
  $insertSQL = "UPDATE gr_alivio SET anio=$anio, numacuerdo=$numacuerdo, descripcion=$descripcion WHERE id_unico = $id";
...
<script type="text/javascript">
  ...
  $("#ver1").click(function(){
    $("#myModal1").modal('hide');
    window.location='../listar_GR_ALIVIO.php';
  });
</script>
<?php }else{ ?>
<script type="text/javascript">
