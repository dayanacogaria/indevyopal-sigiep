<?php
  ...
  $queryUA="SELECT nombre FROM gp_tipo_productor "
          . "WHERE id_unico = '$id'";
  $carA = $mysqli->query($queryUA);
  $numA=  mysqli_fetch_row($carA);
  $cadena2 = strtolower($numA[0]);
  
  $queryU="SELECT * FROM gp_tipo_productor "
 ...
 //comparación para que guarde
   if($nombreC2==$nombreC1){
         ...
         $update = "UPDATE gp_tipo_productor "
              . "SET nombre ='$nombre'  "
              . "WHERE id_unico = '$id'";
...
<html>
    ...
    <?php if($resultado =='3') { ?>
        <script type="text/javascript">
          ...
          $("#ver3").click(function(){
            $("#myModal3").modal('hide');
            window.location='../LISTAR_GP_TIPO_PRODUCTOR.php';
          });
        </script>
    <?php } else { ?>
