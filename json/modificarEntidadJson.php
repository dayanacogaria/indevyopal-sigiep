...
$direccion  = '"'.$mysqli->real_escape_string(''.$_POST['txtDireccion'  ].'').'"';
$telefono   = '"'.$mysqli->real_escape_string(''.$_POST['txtTelefono'   ].'').'"';
$email      = '"'.$mysqli->real_escape_string(''.$_POST['txtEmail'      ].'').'"';
$tipo       = $mysqli->real_escape_string(''.$_POST['sltTipo'].'');
$ubicacion  = $mysqli->real_escape_string(''.$_POST['sltUbicacion'].'');
...
//modificar ne la base de datos
  $insertSQL = "UPDATE gn_entidad SET empleado = $empleado, nombre = $nombre,direccion = $direccion, telefono = $telefono, email = $email, tipo = $tip, ubicacion = $ubi WHERE id_unico = $id";
...
<script type="text/javascript">
  ...
  $("#ver1").click(function(){
    $("#myModal1").modal('hide');
    window.location='../listar_GN_ENTIDAD.php';
  });
</script>
<?php }else{ ?>
<script type="text/javascript">
