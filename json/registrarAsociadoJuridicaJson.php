<?php
  ...
  session_start();    
      ...
      $tipoI  = '"'.$mysqli->real_escape_string(''.$_POST['tipoIdent'].'').'"';
  ...
  if($_POST['contacto']==0 || empty($_POST['contacto']) || $_POST['contacto']=='""'){
    $contacto = 'NULL';
  }else{
    $contacto = '"'.$mysqli->real_escape_string(''.$_POST['contacto'].'').'"';
  }

  if($_POST['zona']==0 || empty($_POST['zona']) || $_POST['zona']=='""'){
    $zona = 'NULL';
  }else{
    $zona = '"'.$mysqli->real_escape_string(''.$_POST['zona'].'').'"';
  }

  $insertSQL = "INSERT INTO gf_tercero (TipoIdentificacion, NumeroIdentificacion, DigitoVerficacion, Sucursal, RazonSocial, TipoRegimen, TipoEmpresa, RepresentanteLegal, CiudadIdentificacion, Contacto, Zona) VALUES( $tipoI, $numId,  $digito, $sucursal,  $razonS, $tipoR,  $tipoE, $repreLegal,  $ciudad,  $contacto, $zona )";
